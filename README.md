...
Bugs were discovered as follows:

1. Manually inspect the results database for anomalies by reviewing log groups and server output.
2. Run `compare_db.py` to diff log groups across runs and surface discrepancies between server behaviours.
3. Run `check_compliance.py` against the database to catch spec violations, particularly for multistep scenario bugs where state-dependent misbehaviour is harder to spot by eye.
...
The following files were generated by Claude Opus 4.7 based on WebTransport drafts and project context:
...
- `src/sequence_mutator.py`

The correctness of these files was verified through manual testing. For other parts of the project, AI was only used to help fix bugs, write short functions, or code segments.
